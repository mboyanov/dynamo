...
def checkParantheses(command):
    leftOpenParantheses = 0
    leftOpenBraces = 0
    for (index, char) in enumerate(command):
        if char == '=' and command[index - 1] != '!' and command[index - 1] != '=' and command[index + 1] != '=' and leftOpenBraces != 0:
            if leftOpenBraces != 0:
                raise ParserException("Improper syntax - Improper nesting of [ and ] in " + command)
        elif char == '=' and command[index - 1] != '!' and command[index - 1] != '=' and command[index + 1] != '=' and leftOpenParantheses != 0:
            if leftOpenParantheses != 0:
                raise ParserException("Improper syntax - Improper nesting of [ and ] in " + command)
        elif char == "(":
            leftOpenParantheses = leftOpenParantheses + 1
        elif char == ")":
            leftOpenParantheses = leftOpenParantheses - 1
        elif char == "[":
            leftOpenBraces = leftOpenBraces + 1
        elif char == "]":
            leftOpenBraces = leftOpenBraces - 1
    if leftOpenBraces != 0:
        raise ParserException("Improper syntax - Improper nesting of [ and ] in " + command)
...
def getClauses(right):
    function=right
    forclause=None
    whereclause=None
    whereindex = re.search("where", right)
    if whereindex is not None:
        whereclause = right[whereindex.end():]
        right=right[:whereindex.start()-1]
    forindex = re.search("for", right)
    if forindex is not None:
        forclause = right[forindex.start():]
        function = right[:forindex.start() - 1]

    return function,forclause,whereclause
...
def checkParameter(parameter, constants=[], arrays={}):
    try:
        ...
        if 'arrays' in parameter:
            ...
            if not found:
                raise ParserException("Unknown array " + parameter) # TODO: is this reacheable???
    return parameter
